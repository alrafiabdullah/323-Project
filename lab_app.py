...
def lab_env_db():
    ...
    print("rendering lab_env_db.html with: %s, %s, %s" %
          (timezone, from_date_str, to_date_str))
    ...
    return render_template("lab_env_db.html",   timezone=timezone,
                           ...
                           query_string=request.query_string[0:len(
...
def get_records():
    ...
    print("REQUEST:")
    print(request.args)
    print("from: %s, to: %s, timezone: %s" %
          (from_date_str, to_date_str, timezone))
    ...
    print("Received from browser: %s, %s, %s, %s" %
          (from_date_str, to_date_str, timezone, range_h_int))
    ...
    print('2. From: %s, to: %s, timezone: %s' %
          (from_date_str, to_date_str, timezone))
...
def to_plotly():
    ...
    for record in temperatures:
        local_timedate = arrow.get(record[0], "YYYY-MM-DD HH:mm").to(timezone)
        time_series_adjusted_temperatures.append(
